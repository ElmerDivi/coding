...
try:
  ...
  # exit when no input is given and this catches EOF(end of file) exception
  sys.exit()

# Initializes N,M,Q to keep track of the sum of n,m,q (should not exceed 3*10**5)
N,M,Q = 0,0,0
...
while(t):
  ...
  # dictionary to keep track of which row,col gets repeated and how many times
  row, col = {},{}
  # intialized to zero
  ...
  while q:
    ...
    try:
      ...
      # if r and c out of matrix dimentions then exit program
    ...
    if rcheck or ccheck:
      ...
      if rcheck and ccheck:
        col[c] += 1
        row[r] += 1
      # else increment r, is the new row, set it to one and increment c by one
      elif ccheck:
        col[c] += 1
        row[r] = 1
      # else increment c, is the new col, set it to one and increment r by one
      elif rcheck:
        row[r] += 1
        col[c] = 1
      # this will not happen when both r and c are new because the condition doesn't allow
      else:
        pass
  ...
  for i in row.values():
    # dictionary is (key,value) pair , for every value in row check if odd increment by one
    if i%2 == 1:
      # counts all odd rows
      num_odd_row += 1
  # same thing for col
